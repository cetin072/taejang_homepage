...
select ok(pg_get_functiondef('public.private_payroll_confirmed_attendance_readiness(date,date)'::regprocedure) ilike '%day_unconfirmed%' and pg_get_functiondef('public.private_payroll_confirmed_attendance_readiness(date,date)'::regprocedure) ilike '%attendance_exception_unresolved%','readiness blocks unconfirmed days and unresolved evidence exceptions');
select ok(pg_get_functiondef('public.private_payroll_confirmed_attendance_readiness(date,date)'::regprocedure) ilike '%employee_record_missing%' and pg_get_functiondef('public.private_payroll_confirmed_attendance_readiness(date,date)'::regprocedure) ilike '%confirmed_duration_invalid%','readiness blocks missing employee records and invalid duration');
...
select ok(pg_get_functiondef('public.private_build_payroll_calculation_input(date,date,uuid)'::regprocedure) ilike '%payroll-db-input-v5-confirmed-native%' and pg_get_functiondef('public.private_build_payroll_calculation_input(date,date,uuid)'::regprocedure) ilike '%confirmed_attendance%','input carries exact confirmed-attendance fingerprint');
