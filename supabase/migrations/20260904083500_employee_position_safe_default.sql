...
begin
  ...
  -- New employee forms use the first option as the initial value. Prefer the
  -- ordinary employee position instead of a high-authority title such as CEO.
  select coalesce(jsonb_agg(jsonb_build_object('id',p.id,'name',p.name,'code',p.code)
    order by case when p.code='general_worker' then 0 else 1 end, p.sort_order, p.name),'[]'::jsonb)
  into positions_json
