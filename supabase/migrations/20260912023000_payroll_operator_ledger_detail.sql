-- Issue #182: enrich the existing lightweight operator context for the practical payroll ledger.
-- No new operator input flow or payroll table is introduced.
begin;
...
begin
  ...
  if month_row.latest_run_id is not null then
    ...
    left join lateral (
      select
        count(*) filter (where ar.auto_decision='unpaid_absence')::integer as absence_day_count,
        count(*) filter (where ar.auto_decision='paid_leave')::integer as paid_leave_day_count,
        count(*) filter (where ar.auto_decision='paid_holiday')::integer as paid_holiday_day_count,
        coalesce(
          jsonb_object_agg(
            to_char(ar.work_date,'YYYY-MM-DD'),
            jsonb_build_object(
              'hours',case
                when ar.auto_decision='confirmed_correction' then coalesce(ar.confirmed_hours,0)
                when ar.auto_decision in ('actual_scheduled','paid_leave','paid_holiday') then coalesce(ar.scheduled_hours,0)
                when ar.auto_decision='unpaid_absence' then 0
                else null
              end,
              'decision',ar.auto_decision,
              'review_status',ar.review_status
            ) order by ar.work_date
          ) filter (
            where ar.auto_decision in (
              'actual_scheduled','confirmed_correction','paid_leave','paid_holiday','unpaid_absence'
            )
          ),
          '{}'::jsonb
        ) as attendance_days
      from public.payroll_attendance_rows ar
      join public.payroll_attendance_import_batches b
        on b.id=ar.batch_id and b.status='accepted'
      where ar.employee_uuid=e.id
        and ar.work_date>=p_payroll_month
        and ar.work_date<(p_payroll_month + interval '1 month')::date
    ) att on true
...
comment on function public.get_payroll_operator_month_context(date) is
  'Lightweight operator month context with payroll ledger attendance counts/day map and statutory/net previews; no additional operator input flow.';
