-- Issue #318: complete trusted persistence support for the already-supported monthly salary runtime.
begin;

alter table public.payroll_employee_results
  drop constraint if exists payroll_employee_results_rate_status_check;

alter table public.payroll_employee_results
  add constraint payroll_employee_results_rate_status_check
  check (rate_status in (
    'single_rate',
    'monthly_salary',
    'missing_rate_review_required',
    'multiple_rates_review_required',
    'monthly_salary_review_required'
  ));

alter table public.payroll_employee_results
  drop constraint if exists payroll_employee_results_partial_gross_withheld_ck;
...
alter table public.payroll_employee_results
  ...
  check (
    ...
    or (
      rate_status in ('single_rate','monthly_salary')
      and unresolved_count=0
      and weekly_holiday_pending_weeks=0
    )
  );

CREATE OR REPLACE FUNCTION public.private_persist_payroll_calculation(p_actor_id uuid, p_payroll_month date, p_cutoff_date date, p_expected_batch_id uuid, p_expected_input_basis_fingerprint text, p_statutory_input_fingerprint text, p_calculation_version text, p_generated_at timestamp with time zone, p_employee_count integer, p_unresolved_item_count integer, p_rate_review_count integer, p_gross_pay_preview numeric, p_gross_pay_preview_status text, p_payable_hours_preview numeric, p_employee_results jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  if coalesce(p_statutory_input_fingerprint,'') !~ '^[0-9a-f]{64}$' then
    raise exception using errcode='22023', message='INVALID_PAYROLL_STATUTORY_INPUT_FINGERPRINT';
  end if;
  ...
  loop
    ...
    if coalesce(result_row ->> 'rate_status','') not in (
      'single_rate','monthly_salary','missing_rate_review_required',
      'multiple_rates_review_required','monthly_salary_review_required'
    ) then
      raise exception using errcode='22023', message='INVALID_PAYROLL_EMPLOYEE_RATE_STATUS';
    end if;
    if coalesce(result_row ->> 'rate_status','') not in ('single_rate','monthly_salary')
  ...
  run_key := md5(
    ...
    current_input_fingerprint || '|' || p_statutory_input_fingerprint || '|' || p_cutoff_date::text
  );
  ...
  where payroll_month_id = month_row.id
    ...
    and statutory_input_fingerprint = p_statutory_input_fingerprint
  ...
  if found then
    ...
    insert into public.payroll_calculation_runs (
      payroll_month_id,run_key,calculation_version,input_fingerprint,statutory_input_fingerprint,cutoff_date,generated_at,
      ...
      month_row.id,run_key,p_calculation_version,current_input_fingerprint,p_statutory_input_fingerprint,p_cutoff_date,p_generated_at,
  ...
  return jsonb_build_object(
    ...
    'statutory_input_fingerprint',p_statutory_input_fingerprint,
