-- Restrict canonical Shadow Payroll calculation scope to employees that have an
-- active, reviewed payroll source identity mapping. Platform employees that are
-- not payroll-scoped must never be pulled into a payroll run implicitly.
begin;
...
begin
  ...
  where t.effective_from <= month_end
    ...
    and exists (
      select 1
      from public.payroll_source_identity_mappings m
      where m.employee_uuid = t.employee_uuid
        and m.status = 'active'
    );
