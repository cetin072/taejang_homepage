-- Staging-applied canonical payroll calculation input builder.
-- Raw clock fields remain evidence and are intentionally excluded from calculation input.
begin;
...
begin
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object('date',h.holiday_date,'name',h.holiday_name,'paid',h.paid)
...
begin
  ...
  perform public.private_append_audit(
    actor_id,'payroll_calculation_input_read','payroll_month',p_payroll_month::text,
    'success','급여 계산 입력 조회',
    jsonb_build_object(
