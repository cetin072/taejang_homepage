-- Issue #268 / audit remediation D: a read-only payslip may expose persisted
-- payroll facts, but must not invent a component split from current formulae.
begin;
...
begin
  ...
  if actor_id is null or not public.private_payroll_operator_allowed() then
    perform public.private_append_audit(
      ...
      'denied', '개인 급여명세서 초안 접근 권한 없음', jsonb_build_object('payroll_month', p_payroll_month)
    );
  ...
  if month_row.latest_run_id is null then raise exception using errcode = '55000', message = 'PAYROLL_DRAFT_RUN_REQUIRED'; end if;
  select * into run_row from public.payroll_calculation_runs where id = month_row.latest_run_id and payroll_month_id = month_row.id;
  if not found then raise exception using errcode = '55000', message = 'PAYROLL_LATEST_RUN_INTEGRITY_ERROR'; end if;
  select * into result_row from public.payroll_employee_results where run_id = run_row.id and employee_uuid = p_employee_uuid;
  ...
  from public.employees e join public.people p on p.id = e.person_id where e.id = result_row.employee_uuid;
  ...
  if result_row.unresolved_count > 0 then review_reasons := review_reasons || jsonb_build_array('unresolved_attendance'); end if;
  if result_row.weekly_holiday_pending_weeks > 0 then review_reasons := review_reasons || jsonb_build_array('weekly_holiday_pending'); end if;
  if result_row.rate_status not in ('single_rate', 'monthly_salary') then review_reasons := review_reasons || jsonb_build_array('rate_review_required'); end if;
  if result_row.gross_pay_preview is null then review_reasons := review_reasons || jsonb_build_array('gross_pay_review_required'); end if;
  if statutory_status <> 'complete' then review_reasons := review_reasons || jsonb_build_array('statutory_deduction_review_required'); end if;
  ...
  perform public.private_append_audit(
    ...
    jsonb_build_object('payroll_month', p_payroll_month, 'run_id', run_row.id, 'employee_uuid', result_row.employee_uuid, 'draft_status', draft_status)
  );

  return jsonb_build_object(
    'kind', 'payslip_draft', 'payroll_month', p_payroll_month, 'status', draft_status, 'review_reasons', review_reasons,
    'run', jsonb_build_object('id', run_row.id, 'generated_at', run_row.generated_at, 'calculation_version', run_row.calculation_version, 'cutoff_date', run_row.cutoff_date),
    ...
    'work_summary', jsonb_build_object(
      ...
      'weekly_holiday_actual_hours', result_row.weekly_holiday_actual_hours,
      'weekly_holiday_expected_hours', result_row.weekly_holiday_expected_hours,
      'weekly_holiday_pending_weeks', result_row.weekly_holiday_pending_weeks,
      'payable_hours_preview', result_row.payable_hours_preview
    ),
    'earnings', jsonb_build_array(jsonb_build_object('code', 'gross_pay_preview', 'label', '총지급 가안', 'amount', result_row.gross_pay_preview)),
    ...
    'totals', jsonb_build_object('gross_pay_preview', result_row.gross_pay_preview, 'statutory_deduction_preview', nullif(statutory ->> 'total', '')::numeric, 'net_pay_preview', case when statutory_status = 'complete' then nullif(statutory ->> 'net', '')::numeric else null end),
    'notice', '급여명세서 초안입니다. 저장된 계산 결과에 별도 항목분해가 없어 총지급 가안만 제공합니다. 재계산·발송·지급·송금·세금 또는 보험 신고를 수행하지 않습니다.'
  );
end;
$$;
...
comment on function public.get_payroll_employee_payslip_draft(date, uuid) is
  'Issue #268: capability-guarded read-only payslip draft from persisted payroll facts only; no read-time component arithmetic, calculation, delivery, payment, remittance, tax filing, or month lock.';
