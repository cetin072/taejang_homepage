-- Staging-applied payroll read context and persisted-basis helper.
begin;
...
begin
  select * into month_row from public.payroll_months where id = p_payroll_month_id;
  ...
  where id = p_run_id and payroll_month_id = month_row.id;
  if not found then raise exception using errcode='55000', message='PAYROLL_RUN_MONTH_MISMATCH'; end if;

  select count(*)::integer into incoming_count
  from public.payroll_adjustments a
  where a.target_month = month_row.payroll_month and a.status <> 'cancelled';
