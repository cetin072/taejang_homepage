-- Issue #312: include statutory input in payroll run idempotency.
begin;
...
begin
  if not exists (
    ...
    where conname='payroll_calculation_runs_statutory_input_fingerprint_check'
      and conrelid='public.payroll_calculation_runs'::regclass
  ) then
    alter table public.payroll_calculation_runs
...
create or replace function public.private_persist_payroll_calculation(
  ...
  p_statutory_input_fingerprint text,
...
revoke all on function public.private_persist_payroll_calculation(
  uuid,date,date,uuid,text,text,text,timestamptz,integer,integer,integer,numeric,text,numeric,jsonb
...
drop function if exists public.private_persist_payroll_calculation(
  uuid,date,date,uuid,text,text,timestamptz,integer,integer,integer,numeric,text,numeric,jsonb
);

comment on column public.payroll_calculation_runs.statutory_input_fingerprint is
  'SHA-256 of canonical server-only statutory deduction input. Changes force a new provisional payroll run without exposing sensitive identity values.';
