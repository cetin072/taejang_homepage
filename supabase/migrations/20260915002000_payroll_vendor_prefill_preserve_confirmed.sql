-- Goal #142 / Issues #210-#211: a later vendor spreadsheet is evidence, not authority to
-- silently replace attendance that an operator already accepted.
--
-- Keep the existing append-only history and permission contract. The only behavioral change is:
-- spreadsheet prefill rows (source_kind=xlsx_prefill) are skipped when the same employee/date
-- already has accepted imported evidence or any saved editor/manual version. Deliberate operator
-- edits remain possible because xlsx_post_edit/manual_ui are still appended normally.
begin;
...
begin
  ...
  loop
    ...
    -- A spreadsheet re-download may report changed source evidence. That must not become a
    -- newer effective attendance version merely because the user pressed Save. Existing accepted
    -- import/manual values stay authoritative until the operator deliberately edits the row.
...
comment on function public.save_payroll_attendance_manual_entries(date,jsonb) is
  'Append-only attendance save. Canonical eligibility is required. Automatic xlsx_prefill cannot supersede already accepted employee/date evidence; explicit xlsx_post_edit/manual_ui can append a new effective version.';
