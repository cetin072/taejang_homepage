-- Issue #182: practical ledger read model.
-- Prefer current calculated statutory values; if absent, use immutable historical as-paid values for display only.
-- This does not rewrite payroll calculation results or historical facts.
begin;
...
comment on function public.get_payroll_operator_ledger_context(date) is
  'Read-only practical payroll ledger context. Current calculated deductions win; immutable historical as-paid values fill missing preview fields without rewriting either source.';
