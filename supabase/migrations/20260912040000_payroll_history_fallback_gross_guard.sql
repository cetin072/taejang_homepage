-- Issue #182: never mix a current Shadow gross amount with historical as-paid deductions/net
-- when the historical gross basis differs. Historical fallback is display-only and is eligible
-- only when the current gross and historical gross match to the won.
begin;
...
begin
  ...
  left join lateral (
    ...
    where h.payroll_month=p_payroll_month
      ...
      and h.record_role='as_paid'
      and h.source_kind='payroll_ledger_confirmed'
    order by h.revision_no desc, h.confirmed_at desc nulls last, h.imported_at desc
    limit 1
  ) hist on true;
...
comment on function public.get_payroll_operator_ledger_context(date) is
  'Read-only practical payroll ledger context. Calculated deductions win. Historical as-paid deduction/net fallback is used only when historical gross matches current Shadow gross; mismatched bases fail closed to review_required.';
