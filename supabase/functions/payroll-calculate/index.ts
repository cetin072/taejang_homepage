...
function safeOperationalLog(event: string, facts: JsonObject) {
  // Never log request/result payloads, names, rates, pay amounts, clocks or HR fields.
  console.info(JSON.stringify({ event, ...facts }));
}

const configuredOrigin = Deno.env.get('PAYROLL_ALLOWED_ORIGIN')?.trim() || DEFAULT_ALLOWED_ORIGIN;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const calculate = createCore({
      ...
      calculationVersion: 'payroll-engine-workweek-golden-v3-statutory-2026',
    });
    ...
    const code = safeErrorCode(error);
    safeOperationalLog('payroll_calculate_failed', { correlation_id: correlationId, code });
