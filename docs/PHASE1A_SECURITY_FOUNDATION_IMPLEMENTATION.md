...
### 2.1 보안 기반 검증 후 첫 사용자 흐름

보안 기반이 `main`에 병합된 뒤에는 별도 기능 PR에서 다음 순서만 구현한다.

- `/staff/`의 시작화면에서 로그인 또는 회원가입 신청을 선택한다. 두 양식은 첫 화면에 함께 길게 노출하지 않는다.
- 가입 시 이름, 업무용 이메일, 비밀번호 확인, 개인정보 수집·이용 동의를 받고 `display_name`만 Auth metadata로 전달한다.
- 로그인과 승인 대기 상태 확인은 항상 `get_my_access_context()`를 사용한다.
- `active` 계정만 `/app/`으로 이동하며, `pending`, `suspended`, `departed`, `deleted`는 내부 화면으로 이동하지 않는다.
- `/app/`은 진입·새로고침·뒤로가기 복원 때마다 Auth session과 현재 access context를 확인한다.
- 다중 역할의 기본 진입 우선순위는 `super_admin` → `ceo` → `operations_manager` → 부서·전문관리 역할 → `field_lead` → 일반 직원 역할 → `general_worker`다. 사용자가 역할을 고르지 않는다.
- 일반 근로자 정보게시판·PWA 설치·QR은 이 흐름 다음 단계에서 구현한다.
