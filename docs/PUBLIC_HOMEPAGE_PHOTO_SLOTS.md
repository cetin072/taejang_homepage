...
## 2026-09 현재 자산 정합화 기록

상태: **확정된 PHOTO 01~08 운영 체계를 확인한 기록**입니다. 이 기록은 PHOTO 01~11 체계를 복원하거나 기존 공개 사진을 교체하는 기획 변경이 아닙니다.

| 분류 | 현재 대상 | 운영 판단 |
| --- | --- | --- |
| 공식 고정 슬롯 | PHOTO 01~08 / `photo-01.webp`~`photo-08.webp` | `assets/js/photo-slots.js`와 이 문서의 연결표를 기준으로 유지합니다. 각 번호는 화면 역할이 생길 때만 사용합니다. |
| 콘텐츠 대표사진 | `TAEJANG_CONTENT.hub`의 `thumbnail`, `thumbnailAlt` | 최근 소식·아카이브의 대표사진입니다. 고정 PHOTO 번호를 부여하지 않습니다. |
| 과거 호환 자산 | `photo-09.webp`~`photo-11.webp` | 초기 PHOTO 01~11 도입 때 함께 저장됐지만, 현재 공개 HTML/CSS/JS에서는 참조되지 않습니다. 삭제하거나 다시 고정 슬롯으로 활성화하지 않고 보존합니다. |
| 현재 직접 노출되지 않는 공식 파일 | `photo-01.webp`, `photo-03.webp` | 공식 슬롯 파일로 보존합니다. 현행 메인에서는 각각 Hero 영상과 정적 환경정비 실사가 역할을 맡아 일반 화면에서 직접 사용하지 않습니다. |

PHOTO 09~11을 새 고정 슬롯으로 복구하거나, 위 보존 파일을 삭제·교체하려면 사진 공개 승인, UX 필요성, 이미지 예산을 별도로 검토하고 사용자 승인을 받습니다.
