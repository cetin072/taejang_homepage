...
### Trigger와 trusted function의 역할 구분

- 일반 클라이언트는 보안 중요 테이블을 직접 변경하지 못하고, 정상 변경은 trusted function 또는 서버 API의 구조화된 결과를 통해 수행합니다.
- trusted function은 advisory lock·행 잠금, 사전 검증, 변경, 성공·거부 감사 기록을 한 경로에서 담당합니다.
- trigger는 직접 SQL 우회나 함수 결함에 대한 DB 최종 방어 계층입니다. trigger의 차단 결과를 일반 API 성공으로 해석하지 않도록 직접 테이블 권한을 회수하고 trusted 경로를 기본으로 합니다.
- 이 두 계층이 실제 PostgreSQL에서 충돌 없이 함께 작동하는지는 아직 확정하지 않았습니다. Phase 1B에서 trigger·function·동시성·거부 감사 기록의 실제 결과를 검증해야 합니다.
