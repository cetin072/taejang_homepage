...
7. 계정 상태와 역할 검사는 화면 숨김이 아니라 서버·데이터·파일 접근 계층에서 강제한다.
8. Supabase는 우선 검토 후보일 뿐, 이 문서만으로 채택·유료 요금제·외부 연결이 확정되지는 않는다.
...
| --- | --- | --- | --- | --- | --- |
| A | 관리자 시스템이 승인 콘텐츠를 정적 JSON 또는 HTML로 만들고 GitHub에 커밋, Netlify가 배포 | 높음. 공개 사이트는 정적본만 사용 | 커밋 이력과 승인 로그를 함께 남길 수 있음. 관리자에 GitHub 쓰기 권한 보호가 핵심 | 중간. 발행 권한을 서버에만 두어야 함 | Git revert·이전 commit으로 명확. GitHub·Netlify 종속, 일반 정적 배포 비용은 낮음 |
...
**후보 A를 우선 설계안으로 검토**합니다. 관리자 앱의 서버 측 발행 구성요소가 승인 revision에서 공개 JSON 또는 정적 콘텐츠를 만들고, 별도 콘텐츠 브랜치 또는 승인된 작업 브랜치에 커밋한 뒤 기존 Netlify 정적 배포 흐름을 이용하는 방식입니다.
...
- 자동 발행 권한을 관리자 앱의 서버 측에만 두고, 직원에게 GitHub·Netlify 권한을 주지 않을 수 있습니다.

다만 아래 결정 전에는 확정하지 않습니다.

- 관리자 시스템의 GitHub 서버 권한 보관 방식
- 같은 저장소 직접 커밋과 별도 콘텐츠 저장소 중 선택
- 발행 커밋을 곧바로 `main`에 둘지, 사람이 검토하는 발행 PR로 둘지
- 일반 발행 빈도와 긴급 정정 절차
...
| `deleted` | 차단 | 무효화 | 전부 차단 | 전부 차단 | 최소 보존·예외 절차 |
...
```mermaid
flowchart TD
  A["퇴사·업무 종료 확인"] --> B["작성 중·검토·예약·파일 담당 목록 확인"]
  B --> C["활성 직원에게 담당 콘텐츠 재배정"]
  C --> D["계정 상태를 departed로 변경"]
  D --> E["세션·refresh token·API·Storage 접근 즉시 차단"]
  E --> F["작성자·revision·감사 로그 보존 확인"]
  F --> G["처리자·시각·최소 사유·재배정 결과 기록"]
  G --> H["차단·재배정 재확인"]
```

### 기존 세션 무효화 방식

세부 API는 선택한 인증 서비스의 공식 문서를 확인한 뒤 확정하지만, Phase 1은 아래 결과를 보장해야 합니다.

1. 상태 변경 트랜잭션이 완료되면 해당 사용자의 현재 세션과 refresh token을 서버 측에서 무효화합니다.
2. 상태 변경 직전 열려 있던 브라우저도 다음 요청에서 `active` 검사에 실패하여 저장·수정·삭제·검토·승인·발행·업로드를 수행할 수 없습니다.
3. 클라이언트는 로그인 화면으로 이동하되, UI 이동 실패 여부와 관계없이 서버·DB·Storage가 거부합니다.
4. 복구는 기존 세션을 되살리지 않고, 역할과 상태를 재확인한 뒤 새 로그인으로 시작합니다.
5. 세션 무효화 성공·실패와 후속 차단 확인을 감사 로그에 남깁니다.
...
- 퇴사 처리 완료 전 재배정이 실패하면 상태 전환을 무조건 취소하지는 않되, 차단은 우선 적용하고 관리자에게 즉시 미배정 업무를 경고합니다. 후속 재배정 책임자와 기한을 기록합니다.
...
로그에는 비밀번호, access token, refresh token, 비밀키, 동의서 원문, 계약 원문, 민감한 본문 전문을 저장하지 않습니다. 보존 기간과 로그 열람자는 사용자 결정 사항입니다.
