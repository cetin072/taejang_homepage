...
- 상단·모바일·푸터의 소식 진입점은 `소식·기록` 하나만 사용
...
`activities.html`은 삭제하지 않습니다. 기존 게시물 상세주소인 `activities.html?id=...`가 계속 작동하도록 유지하며, 이 페이지에서 상단 메뉴의 현재 위치는 `소식·기록`으로 표시합니다. 메인 환경·사회공헌 카드도 `archive.html`로 연결합니다.

향후 환경·사회공헌 콘텐츠가 충분히 쌓이면 별도의 사업 소개 페이지를 검토할 수 있지만, 날짜순 소식 목록을 다시 독립 메뉴로 만들지는 않습니다.
...
- `activities.html` 상세경로와 `소식·기록` 연결이 유지되는지 확인
...
- 개인정보처리방침·이용약관 링크 확인
- 회사소개·기업협력·일터·소식·기록 링크 확인
- 공개 메뉴에 `태장의 활동`이 별도 항목으로 표시되지 않는지 확인
