...
- 원본 HTML에 이전 이메일 `taejang2025@naver.com`이 남아 있지 않은지 확인
- 원본 HTML에 이전 메뉴명 `콘텐츠 소식`이 남아 있지 않은지 확인
- 자바스크립트를 실행하지 않은 원본에서도 임직원·자료실 링크가 숨겨지는지 확인
- 내부 링크 대상 파일 존재 여부
- 새 탭 링크의 `noopener` 적용
- 페이지별 `main`·`h1`·중복 `id` 검사
- PHOTO 01~11 연결, 검수 모드, 모바일 크롭 설정
- 최근 활동 빈 상태와 최대 3개 표시 회귀 검사

`assets/js/site.js`의 이전 이메일 변환 코드는 오래된 캐시나 예외 페이지를 위한 보조 안전장치로만 유지합니다. 공개 HTML 원본은 새 이메일과 현재 메뉴명으로 직접 통일합니다.
...
기존 `taejang2025@naver.com` 주소는 일정 기간 병행 운영하며, 신규 메일 전달과 회신 서명을 통해 새 주소로 자연스럽게 전환합니다. 기존 주소는 홈페이지 공개 HTML에는 표시하지 않습니다.
