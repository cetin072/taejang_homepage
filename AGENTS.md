...
## Existing-First / 오픈소스 우선 개발

- 범용 기능을 새로 만들기 전에 [`docs/operations/OPEN_SOURCE_ADOPTION.md`](docs/operations/OPEN_SOURCE_ADOPTION.md)를 기준으로 기존 태장 Core → 내부 다른 프로젝트의 검증된 구현 → 공식 SDK/API → 검증된 오픈소스 순으로 먼저 확인합니다.
- 오픈소스 도입은 라이선스, 유지보수 상태, 보안, 성능·배터리, 접근성, 통합 난이도와 총비용을 함께 검토합니다.
- 외부 공급자나 엔진의 실제 교체 가능성이 있는 경계에서만 Adapter/Service를 두고, 태장 고유 업무규칙과 안정적인 기존 Core는 추상화 목적만으로 재작성하지 않습니다.
