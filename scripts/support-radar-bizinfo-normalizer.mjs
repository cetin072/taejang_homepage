...
function positiveInteger(value, field) {
  ...
  if (!Number.isInteger(number) || number <= 0) {
    throw new Error(`BIZINFO_${field.toUpperCase()}_INVALID`);
  }
  return number;
}

export function buildBizinfoPublicRequestPlan({
  dataType = 'json',
  searchCnt = null,
  searchLclasId = null,
  hashtags = null,
  ...
  if (!['json', 'rss'].includes(normalizedType)) throw new Error('BIZINFO_DATA_TYPE_INVALID');
  ...
  const category = clean(searchLclasId);

  if (count !== null) params.searchCnt = String(count);
  if (category) {
    if (!BIZINFO_CATEGORY_CODES.has(category)) throw new Error('BIZINFO_SEARCH_LCLAS_ID_INVALID');
  ...
  return {
    ...
    value: {
      ...
      notice: {
        ...
        application_start_precision: period.start_date ? 'date' : 'unknown',
        deadline_date: period.end_date,
        deadline_precision: period.end_date ? 'date' : 'unknown',
...
export function normalizeBizinfoPayload(payload) {
  ...
  const totalCount = /^\d+$/.test(reportedTotal) ? Number(reportedTotal) : null;
