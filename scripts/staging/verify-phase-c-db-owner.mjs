...
try {
  ...
  const result = await databaseQuery(config, required(DB_OWNER_TOKEN), `
    ...
    select
      ...
      (select count(distinct procedure.proname) from pg_proc procedure join pg_namespace namespace on namespace.oid = procedure.pronamespace where namespace.nspname = 'public' and procedure.proname = any(array['save_promotion_draft', 'submit_promotion_revision', 'review_promotion_revision', 'queue_promotion_revision', 'get_my_promotion_workspace', 'list_promotion_public_export_candidates'])) as promotion_functions;
  `);
