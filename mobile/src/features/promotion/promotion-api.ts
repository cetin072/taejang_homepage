...
export async function savePromotionDraft(client: PlatformSupabaseClient, input: PromotionDraftInput) {
  ...
  const { data, error } = await client.rpc('save_promotion_draft', {
    ...
    p_external_url: input.contentType === 'external_content' ? nullable(input.sourceReferenceUrl) : null,
    p_byline: null,
