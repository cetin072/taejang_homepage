(() => {
  ...
  const state={currentNoticeId:null,loading:false,saving:false};
  ...
  function reviewKindLabel(value){return ({initial:'초기 검토',eligibility:'자격 검토',application:'신청 준비 검토',result:'결과 검토',other:'기타 검토'})[value]||value||'검토';}

  async function markReviewed(kind,note){
    if(!canMark()||!state.currentNoticeId||state.saving)return;
    state.saving=true;
    try{
  ...
  async function inject(){
    ...
    try{
      ...
      if(!data.review_count){panel.append(text('p','아직 사람이 검토 완료로 표시한 기록이 없습니다.','support-radar-warning'));}
        ...
        if(data.latest_review){panel.append(text('p',`${data.latest_review.reviewer_name||'담당자'} · ${reviewKindLabel(data.latest_review.review_kind)}${data.latest_review.note?` · ${data.latest_review.note}`:''}`,'support-radar-note'));}
      }
      if(canMark()){
