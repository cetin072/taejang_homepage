(() => {
  ...
  function requestCard(profile, options, employees) {
    ...
    form.append(field('실제 직원', employee), linkedSummary, field('업무 권한', role), field('처리 사유', reason));

    const actions = el('div', null, 'quick-links');
    const approveButton = button('직원 연결 후 가입 승인', () => approve(profile, employee, role, reason, card));
    approveButton.dataset.approveSignup = '1'; actions.append(approveButton); card.append(form, actions);
    return card;
  }
  ...
  async function openAccountApproval() {
    ...
    try {
      ...
      intro.append(
        ...
        el('h2', '임직원 계정 승인'),
        el('p', '가입 신청을 실제 직원 마스터와 연결한 뒤 업무 권한을 부여합니다. 이름이나 이메일로 자동매칭하지 않습니다.')
      );
  ...
  function syncDashboard() {
    ...
    card.append(el('p', '신규 임직원 계정 신청을 실제 직원 마스터와 연결한 뒤 승인합니다.'));
    card.append(button('가입 승인 열기', openAccountApproval, true)); grid.prepend(card);
  }
