...
test('central navigation groups menus by work category without unfinished manager manuals', () => {
  ...
  assertOrdered(leadBlock, ['대시보드', '새 홍보글 작성', '승인·검토', '기존 글 관리', '홈페이지 내용 관리', '공지 관리', '팀 직원 관리', '신규 직원 등록 요청', '업무 배정', '일정 관리', '출근부', '홈페이지', '신규 사업 기획']);
  assert.doesNotMatch(leadBlock, /공개글 관리|미발행 글 삭제|상시 안내 관리/);
