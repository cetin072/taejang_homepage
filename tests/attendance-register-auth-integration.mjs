...
async function linkedWorker(label, role='general_worker') {
  const account = await signup(label,null);
...
assert.equal((await save(parameters,c,`${day}T09:10:00+09:00`)).code,'STALE_ATTENDANCE','old blank form cannot overwrite a new record');
c = await context();
assert.equal(c.reason_required,true);
assert.equal((await save(parameters,c,`${day}T09:05:00+09:00`)).code,'REASON_REQUIRED');
assert.equal((await save(parameters,c,`${day}T09:00:00+09:00`)).code,'ATTENDANCE_UNCHANGED');
...
for (const account of [worker,tech,pending]) {
  ...
  assert.equal((await save(parameters,await context(),`${day}T09:20:00+09:00`,'권한 없는 변경 시도',account.token)).code,'FORBIDDEN');
}
...
assert.ok(!(await api('/rest/v1/attendance_corrections?select=*',lead.token,null,'GET')).ok,'direct table read remains denied');
...
// Raw GPS evidence remains unchanged; corrected output is exposed to the worker.
...
const rawQuery = `select row_to_json(ae)::text from public.attendance_events ae where profile_id='${worker.id}'::uuid and work_date='${today}' and event_type='clock_in'`;
const before = sql(rawQuery);
...
assert.equal(sql(rawQuery),before);
assert.equal((await rpc('get_my_attendance_today',worker.token)).data.clock_in.status,'corrected');

const simulation = await rpc('set_role_simulation_mode',ops.token,{p_role_code:'promotion_staff'});
assert.equal(simulation.data.code,'ROLE_SIMULATION_SET');
assert.equal((await rpc('create_attendance_correction',ops.token,{...parameters,p_action:'set_time',p_corrected_event_at:`${day}T09:00:00+09:00`,p_reason:'권한 시뮬레이션 차단'})).data.code,'FORBIDDEN');
const restore = await rpc('set_role_simulation_mode',ops.token,{p_role_code:'actual'});
assert.equal(restore.data.code,'ROLE_SIMULATION_CLEARED');
