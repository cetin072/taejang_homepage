...
assert.equal(Number(payslip.data?.work_summary?.weekly_holiday_hours), 8);
assert.equal(Number(payslip.data?.earnings?.find(item => item.code === 'base_pay_preview')?.amount), 920000);
assert.equal(Number(payslip.data?.earnings?.find(item => item.code === 'weekly_holiday_pay_preview')?.amount), 80000);
