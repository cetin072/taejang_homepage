...
for (const [filename, html] of Object.entries(pages)) {
  ...
  assert.doesNotMatch(html, /콘텐츠 소식/, `${filename} 원본에 이전 메뉴명을 남기지 않습니다`);
...
assert.equal((partnership.match(/class="partnership-visual-card"/g) || []).length, 3);
...
assert.doesNotMatch(resources, /taejang2025@naver\.com|콘텐츠 소식|href="staff\//);
...
for (const [filename, html] of Object.entries(pages).concat([['privacy.html', privacy], ['terms.html', terms], ['404.html', notFound]])) {
