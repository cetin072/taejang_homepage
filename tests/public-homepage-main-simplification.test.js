...
assert.match(index, /class="illustration-slot illustration-slot--image"[\s\S]*assets\/images\/illustrations\/taejang-illustration-d\.webp[\s\S]*농업 기반에서 사람의 강점과 맞춤 직무를 거쳐 사업으로 이어지는 모습/);
...
assert.match(business, /class="business-cards"[\s\S]*<\/div><figure class="illustration-slot illustration-slot--image"[\s\S]*assets\/images\/illustrations\/taejang-illustration-e\.webp[\s\S]*필요한 업무를 작업 구조와 맞춤 역할로 연결해 운영하는 모습[\s\S]*class="business-workflow"/);
...
assert.equal((index.match(/지속 가능한/g) || []).length, 2, 'Hero 제목과 회사 정의 보조문구에서 사용합니다');
...
assert.match(about, /class="glance-grid"[\s\S]*<\/div>\s*<figure class="illustration-slot illustration-slot--image"[\s\S]*assets\/images\/illustrations\/taejang-illustration-c\.webp[\s\S]*참여기업과 태장, 사람에게 맞는 직무와 실제 사업이 연결되는 모습[\s\S]*태장이라는 이름/);
...
assert.match(partnership, /현장 규모와 조건에 따라 사전 준비기간이 달라질 수 있습니다\.[\s\S]*<figure class="illustration-slot illustration-slot--image"[\s\S]*assets\/images\/illustrations\/taejang-illustration-f\.webp[\s\S]*지역의 필요를 함께 실행하고 다음 협력으로 이어가는 모습[\s\S]*현재 환경정비 활동 운영 과정/);
