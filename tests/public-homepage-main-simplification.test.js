...
assert.match(index, /class="illustration-slot"[\s\S]*ILLUSTRATION SLOT[\s\S]*농업 기반 → 사람의 강점 → 맞춤 직무 → 지속 가능한 사업[\s\S]*제작 후보/);
...
assert.match(business, /class="business-cards"[\s\S]*<\/div><aside class="illustration-slot"[\s\S]*ILLUSTRATION SLOT[\s\S]*기업의 필요한 업무 → 작업 구조화 → 맞춤 역할 → 운영·검수·결과[\s\S]*제작 후보[\s\S]*class="business-workflow"/);
...
assert.equal((index.match(/지속 가능한/g) || []).length, 3, 'Hero 제목, 회사 정의 보조문구와 D 일러스트 슬롯 주제에서 사용합니다');
...
assert.match(about, /class="glance-grid"[\s\S]*<\/div>\s*<aside class="illustration-slot"[\s\S]*ILLUSTRATION SLOT[\s\S]*참여기업 4개사 → 태장 → 맞춤 직무 → 실제 사업과 일자리[\s\S]*후속 일러스트 예정[\s\S]*태장이라는 이름/);
...
assert.match(partnership, /현장 규모와 조건에 따라 사전 준비기간이 달라질 수 있습니다\.[\s\S]*<aside class="illustration-slot"[\s\S]*ILLUSTRATION SLOT[\s\S]*지역의 필요 → 기업·기관과 태장 협의 → 함께 실행 → 다음 협력으로 확장[\s\S]*제작 후보[\s\S]*현재 환경정비 활동 운영 과정/);
